...
class User(Base):
    ...
    id = Column(Integer, primary_key=True, unique = True)  # Unique identifier for each user
    username = Column(String(50), unique=True, nullable=False)  #  username
    password = Column(String(128), nullable=False)  #  password
...
class RunHistory(Base):
    ...
    id = Column(Integer, primary_key=True, unique=True)  # Unique identifier for each run
...
class DBManager:
    ...
    def save_run_history(self, username, automata_data, history_data, description=""):
        ...
        rh = RunHistory(
            username=username,
            automaton_json=json.dumps(automata_data),
    ...
    def list_run_histories(self, username):
        ...
        for r in records:
            output.append(
                (r.id, r.description, r.automaton_json, r.history_json))
        sess.close()
