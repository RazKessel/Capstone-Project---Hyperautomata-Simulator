...
from components.utils.constants import STATE_RADIUS, COLOR_BLACK, COLOR_WHITE
from components.utils.logger import operation_logger

class State:
    """ Represents a single GUI state with coordinates, radius, flags, IDs, transitions. """
    ...
    def draw(self, canvas):
        ...
        self.canvas_id = canvas.create_oval(
            self.x - r, self.y - r,
            self.x + r, self.y + r,
            fill=COLOR_WHITE, outline=COLOR_BLACK, width=2,
